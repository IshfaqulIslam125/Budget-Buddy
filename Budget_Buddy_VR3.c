...
void read_balance()
{
    ...
    while (fscanf(file3, "%s %lf", stored_username, &current_balance) == 2)
    {
        ...
        {
            new_user.balance = current_balance; // We update the balance for the user
            found = 1;
            break;
        }
    }
    ...
    if (!found)
    {
        new_user.balance = 0.0; // If the user is not found then we keep the balance zero
    }
...
////////////////////////////////////////////////////////////// USER REGISTRATION ////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        if (option == 1) 
        {
            ...
            if (file4 != 0)                   // If the file is not empty then the condition works
            {
...
///////////////////////////////////////////////////// USER LOGIN ///////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        if (option == 2)
        {
            ...
            file4 = fopen("User_Info.txt", "r"); // We Open the file before entering the loop
            ...
            while (fscanf(file4, "%s %s", user[total_user].username, user[total_user].password) == 2) // Read user info from file
            {
                if (strcmp(user[total_user].username, entered_username) == 0 && strcmp(user[total_user].password, entered_password) == 0)
                {
                    ...
                    while (1) // We run an infinity loop here
                    {
                        printf("╔══════════════════════════════════╗\n");
                        printf("║∘₊✧──────✧₊ Budget Menu ₊✧─────✧₊∘║\n");
                        printf("║ 1. Add Income                    ║\n");
